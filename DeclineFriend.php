...
if ($checkToken == 1) {
    ...
    $sql = "DELETE FROM friends WHERE BINARY ownerUsername = '$friendUsername' AND BINARY friendUsername = '$username'";
    if ($con->query($sql) == TRUE) {
        $Data->status = 200;
        $Data->message = "Decline friend successful!";
        $Data->data = $data;
    } else {
        $Data->status = 402;
        $Data->message = "decline friend failed.";
    }
