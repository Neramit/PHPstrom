...
if ($checkToken == 1) {
    ...
    $sql = "UPDATE group_member SET memberStatus = 1 WHERE BINARY groupUID = '$groupUID' AND BINARY memberUsername = '$username'";
    if ($con->query($sql) == TRUE) {
        $Data->status = 200;
        $Data->message = "Join group successful!";
        $Data->data = $data;
    } else {
        $Data->status = 402;
        $Data->message = "Join group failed.";
    }
